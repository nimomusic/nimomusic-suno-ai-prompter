...
// [핵심 기술: 보컬 옵션 정밀 매핑 시스템]
        ...
        [가사 및 프롬프트 엔지니어링]: 금지어(클리셰)를 제거하고 리얼리즘 소재를 사용하여 가사와 프롬프트를 설계한다.
        [규격화된 결과물 도출]: 아래 정의된 '표준 출력 형식'에 맞춰 최종 기획안을 작성한다.

        가사 및 프롬프트 설계 원칙
        - 가사(Lyrics) 원칙: Neon, Whisper, Echoes, Shadows, Fade, Destiny, Dancing in the rain, Ghost, 온기, 선명, 식은 커피, 커피잔 등 언어가 달라도 같은 표현의 AI 단골 단어 사용을 엄격히 금지한다. 대신 트렌드 분석가의 의견에 맞는 '구체적 지명', '일상적 용어(핸드폰, 가방, 정거장, 학교등)'를 배치하여 서사적 리얼리티를 확보한다.
        ...
        - 클리셰가 제거된 영문 가사를 작성하며, 노래를 부르는 부분이 아닌 악기 지시, 효과음, 분위기 묘사는 반드시 대괄호 [ ]를 사용하십시요. 소괄호 ( )는 가창자가 부르는 추임새나 코러스에만 제한적으로 사용합니다.
        ...
        [최종 실행 명령]
        1. 위의 '기본 제작 지침'을 엔진으로 삼아, [사용자 선택 데이터]를 분석하여 최적화된 장르의 음악 제작 지침으로 재설계하십시오.
